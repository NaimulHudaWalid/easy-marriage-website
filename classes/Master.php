...
class Master extends DBConnection
{
	...
	function save_product()
	{
		...
		if ($save) {
			if (isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0) {
				if (!is_dir(base_app . 'uploads/shop_' . $id)) {
					mkdir(base_app . 'uploads/shop_' . $id);
					$data = " `upload_path`= 'uploads/shop_{$id}' ";
				} else {
					$data = " `upload_path`= 'uploads/shop_{$id}' ";
				}
				$this->conn->query("UPDATE `items` set {$data} where id = '{$id}' ");
				foreach ($_FILES['img']['tmp_name'] as $k => $v) {
					move_uploaded_file($_FILES['img']['tmp_name'][$k], base_app . 'uploads/shop_' . $id . '/' . $_FILES['img']['name'][$k]);
				}
			}
	...
	function delete_product()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			if (is_dir(base_app . 'uploads/shop' . $id)) {
				$file = scandir(base_app . 'uploads/shop' . $id);
				foreach ($file as $img) {
					if (in_array($img, array('..', '.')))
						continue;
					unlink(base_app . 'uploads/shop' . $id . '/' . $img);
				}
				rmdir(base_app . 'uploads/shop' . $id);
			}
